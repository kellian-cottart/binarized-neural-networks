...
def train_iteration(trial):
    ...
    prior_attaction = trial.suggest_float(
        "prior_attraction", 1e-10, 1e-4, log=True)
    ### LAMBDA PARAMETERS ###
    # suggest int
    n_samples_backward = trial.suggest_categorical("n_samples_backward", [10])
    init_law = trial.suggest_categorical("init_law", ["gaussian"])
    init_param = trial.suggest_float("init_param", 0.1, 10, step=0.1)
    temperature = trial.suggest_float("temperature", 1e-8, 1, log=True)
    ...
    data = {
        ...
        "optimizer_parameters": {
            ...
            "num_mcmc_samples": 10,
            "scale": prior_attaction,
            "init_lambda": init_param,
        },
        "task": task,
        "n_tasks": n_tasks,
        "n_classes": n_classes,
    ...
    net_trainer = trainer.BayesTrainer(
