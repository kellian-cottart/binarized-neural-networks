...
def train_iteration(trial):
    ...
    metaplasticity = trial.suggest_float("metaplasticity", 1, 10, step=0.1)
    weight_decay = trial.suggest_float("weight_decay", 1e-9, 1e-6, log=True)
    width = trial.suggest_float("width", 0.2, 1.4, step=0.1)
    init_param = trial.suggest_float("init_param", 0.01, 0.2, step=0.01)
