...
class MESU(Optimizer):
    ...
    def __init__(self, params, lr_sigma=1, lr_mu=1, mu_prior=0.1, N_mu=1e5, N_sigma=1e5, clamp_grad=0):
        defaults = dict(
            mu_prior=mu_prior,
            N_mu=N_mu,
            N_sigma=N_sigma,
    ...
    def step(self,):
        ...
        for group in self.param_groups:
            ...
            # create a step for each group
            if "step" not in group:
                group["step"] = 0
                # sigma prior is the same as all sigmas (array)
                sigma_p = []
                for p in params_with_grad[::2]:
                    sigma_p.append(p.data)
                group["sigma_prior"] = sigma_p
            group["step"] += 1
...
def mesu(params: List[Tensor], d_p_list: List[Tensor], sigma_prior: float, mu_prior: float, N_mu: int, N_sigma: float, lr_mu: float, lr_sigma: float, clamp_grad: float = 0):
    ...
    for sigma, mu, grad_sigma, grad_mu, sigma_prior in zip(params[::2], params[1::2], d_p_list[::2], d_p_list[1::2], sigma_prior):
