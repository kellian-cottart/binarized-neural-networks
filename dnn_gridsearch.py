...
STUDY = "gridsearch/bimu_1_(no_prior)"
ALL_GPU = True
...
def train_iteration(trial):
    ...
    model = models.BiNNBayesianNN(
        ...
        activation_function=Sign.apply,  # Activation function => None gives sign activation
        output_function="log_softmax",  # Output function
        lambda_init=0,  # Std of lambdas weights
    )

    ### PARAMETERS ###
    lr = trial.suggest_float("lr", 0.01, 1000, log=True)
    ...
    if config["optimizer"] == "BinaryMetaplasticUncertainty":
        optimizer = BinaryMetaplasticUncertainty
    else:
        raise ValueError(
    ...
    bayes_trainer = trainer.GPUTrainer(
        model=model,
        optimizer=optimizer,
